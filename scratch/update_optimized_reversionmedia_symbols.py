...
def updateOptimizedReversionMediaSymbols():
    csvPath = "/Volumes/TimeMachine/ATALAia/Sentinel/backtesting/reversionmedia_grid_results_best.csv"
    strategyName = "ReversionMedia"
    ...
    try:
        ...
        # Asegurar que strategyConfig tenga insertada la estrategia ReversionMedia y esté habilitada
        cursor.execute("""
            INSERT INTO strategyConfig (strategy, enabled, min_rr, min_confidence)
            VALUES (%s, TRUE, 2.0, 70)
        ...
        for _, row in dfBest.iterrows():
            ...
            params = {
                "lrcPeriod": int(row['lrcPeriod']),
                "lrcDev": float(row['lrcDev']),
        ...
        conn.commit()
        logger.info("🎉 Proceso de actualización de parámetros ReversionMedia finalizado con éxito.")
...
if __name__ == "__main__":
    updateOptimizedReversionMediaSymbols()
