...
def updateOptimizedSniperSymbols():
    csvPath = "/Volumes/TimeMachine/ATALAia/Sentinel/backtesting/sniper_grid_results_best.csv"
    strategyName = "Sniper"
    ...
    try:
        ...
        # Asegurar que strategyConfig tenga insertada la estrategia Sniper y esté habilitada
        cursor.execute("""
        ...
        if not dfBest.empty:
            for _, row in dfBest.iterrows():
                ...
                params = {
                    "probaThresholdLong": float(row['probaThresholdLong']),
                    "probaThresholdShort": 1.0 - float(row['probaThresholdLong']),
                    "minConfidence": float(row['minConfidence']),
        ...
        conn.commit()
        logger.info("🎉 Proceso de actualización de parámetros Sniper finalizado con éxito.")
