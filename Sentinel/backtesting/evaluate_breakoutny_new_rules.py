...
def run_backtest_for_time(sh, sm, name):
    print(f"\\n--- EVALUANDO {name} (Inicio NY Time: {sh:02d}:{sm:02d}) ---")
    rd = 15
    rr = 1.0
    tw = 120
    ...
    for sym in symbols:
        ...
        for d in days:
            ...
            bull_breaks = df_post['close'] > rangeHigh
            bear_breaks = df_post['close'] < rangeLow
            
            break_idx = -1
            direction = None
            
            for i in range(len(df_post)):
                if bull_breaks.iloc[i]:
                    if i > 0 and df_post['close'].iloc[i-1] > rangeHigh: continue
                    break_idx = i
                    direction = 'LARGO'
                    break
                elif bear_breaks.iloc[i]:
