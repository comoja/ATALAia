"""
MASTER ORCHESTRATOR 7-DAY COMPONENT BACKTEST - SENTINEL
======================================================
1. Vacía la tabla symbolNotStrategia en MySQL para recalibración limpia semanal.
2. Carga dinámicamente las estrategias activas (enabled = TRUE) de strategyConfig.
3. Carga 45 días de historial para cálculo exacto de indicadores (EMA200, rolling peaks) en 5m/15m/30m/1H/4H.
4. Simula de forma real compounding compuesto trade-a-trade sobre el capital inicial de $418.19 USD.
5. Filtra las operaciones a los últimos 7 días aplicando el blindaje operativo.
6. Identifica los combos perdedores semanales reales e inyecta de forma masiva en symbolNotStrategia en MySQL.
7. Genera los CSVs del backtesting de 7 días y el reporte PDF premium final de auditoría semanal.
"""
import sys
import os
import pandas as pd
...
def clearExclusionsTable() -> None:
    ...
    try:
        print("🧹 Vaciando la tabla symbolNotStrategia en MySQL para recalibración semanal de 7 días...")
...
def saveNewExclusionsToDb(exclusions: list) -> None:
    ...
    if not exclusions:
        ...
        return
    try:
        print(f"📥 Guardando {len(exclusions)} exclusiones semanales en MySQL...")
        conn = dbConnection.getConnection()
        if conn is None:
            return
        cur = conn.cursor()
...
def runTrimestralPortfolioBacktest() -> None:
    print("==========================================================")
    print("  BACKTESTING SEMANAL COMPUESTO (7 DÍAS) - SENTINEL       ")
    print("==========================================================")

    # 1. Vaciar tabla de exclusiones para recalibración semanal limpia
    clearExclusionsTable()

    rawSymbols = dbManager.getSymbols()
    # Usar strictly los símbolos activos en DB
    ...
    # 45 días de historial para cálculo exacto de indicadores
    ...
    if dfRawTrades.empty:
        print("⚠️ No se registraron trades en la simulación de 7 días para ninguna combinación.")
        return
    ...
    for _, row in incompatibles.iterrows():
        dbExclusions.append((
            row['symbol'],
            row['strategy'],
            f"Filtro Semanal: Pérdida neta de ${row['PnL_Total']:.2f} USD en Backtest 7D (Compounding)"
        ))
    ...
    print("✅ CSVs semanales de 7 días generados correctamente.")

    # Generar Reporte PDF Semanal Premium (Verde Esmeralda y Oro)
...
class WeeklyReportPdf(FPDF):
    def header(self) -> None:
        if self.page_no() > 1:
            ...
            self.cell(0, 10, "SISTEMA ATALAIA / SENTINEL - AUDITORÍA SEMANAL DE CONFIANZA OPERACIONAL (7 DÍAS)", 0, 0, 'L')
...
def generateWeeklyReportPdf(dfTrades: pd.DataFrame, dfComboPerf: pd.DataFrame, dfStratPerf: pd.DataFrame, incompatibles: pd.DataFrame, desactivarEstrategias: pd.DataFrame, balanceFinal: float, activeSymbols: list, enabledStrategies: list) -> None:
    try:
        pdf = WeeklyReportPdf(orientation='P', unit='mm', format='letter')
        ...
        pdf.set_y(78)
        pdf.multi_cell(0, 12, "AUDITORÍA OPERATIVA SEMANAL\nPERÍODO DE MERCADO DE 7 DÍAS\nBLINDAJE Y CALIBRACIÓN DE PORTAFOLIO", 0, 'C')
        ...
        pdf.cell(0, 7, "Cierre de Balance Semanal 7D", 0, 1, 'C')
        ...
        kpis = (
            ...
            f"Combos Semanales Excluidos en MySQL (PnL < 0): {len(incompatibles)} parejas."
        )
        ...
        pdf.cell(0, 8, "VERSIÓN DE PRODUCCIÓN - BLINDAJE SEMANAL OPERATIVO M5/M15/1H/4H - CONFIDENCIAL", 0, 1, 'C')

        # PÁGINA 2: RENDIMIENTO POR ESTRATEGIA OPTIMIZADO
        pdf.add_page()
        pdf.set_text_color(24, 28, 36)
        pdf.set_font('Helvetica', 'B', 14)
        pdf.cell(0, 10, "1. AUDITORIA DE ESTRATEGIAS SEMANALES (7 DÍAS)", 0, 1, 'L')
        pdf.ln(3)

        pdf.set_font('Helvetica', 'B', 10)
        pdf.set_text_color(15, 115, 85)
        pdf.cell(0, 6, "[A] RENDIMIENTO DE LAS ESTRATEGIAS EN LA SEMANA", 0, 1, 'L')
        pdf.ln(2)
        ...
        pdf.cell(0, 6, "[B] RECOMENDACIÓN DE FILTRADO TÁCTICO SEMANAL", 0, 1, 'L')
        pdf.ln(1)
        ...
        if not desactivarEstrategias.empty:
            recsStr = "El backtest semanal de 7 días con velas reales en base de datos ratifica que las siguientes estrategias acumulan pérdidas en la semana de mercado real y se recomienda inhabilitar en strategyConfig:\n\n"
            for _, row in desactivarEstrategias.iterrows():
                recsStr += f"• **{row['strategy']}**: PnL de **${row['PnL_Total']:.2f} USD** ({row['Total_Trades']} trades, WR: {row['Win_Rate_%']:.1f}%)\n"
        else:
            recsStr = "Ninguna estrategia en general consolidó pérdidas globales durante la semana evaluada."
        ...
        pdf.set_text_color(24, 28, 36)
        pdf.set_font('Helvetica', 'B', 14)
        pdf.cell(0, 10, "2. BLINDAJE SEMANAL DE PORTAFOLIO EN MYSQL (7 DÍAS)", 0, 1, 'L')
        pdf.ln(3)

        pdf.set_font('Helvetica', 'B', 10.5)
        pdf.set_text_color(15, 115, 85)
        pdf.cell(0, 6, "[A] DIAGNÓSTICO SEMANAL DEL PORTAFOLIO", 0, 1, 'L')
        pdf.ln(2)
        ...
        comparativaText = (
            f"La simulación semanal de **7 Días** con velas de 5 minutos reales e indexadas, "
            f"aplicando compounding compuesto a nivel de portafolio y recalibración de fin de semana, "
            f"convalida la fortaleza estructural del portafolio semanal:\n\n"
            f"1. **Rendimiento Compuesto Semanal:** Con un balance compuesto, el portafolio cerró la semana en "
            f"**${balanceFinal:.2f} USD**, logrando un retorno neto compuesto de **${totalPnl:+.2f} USD ({retornoTotal:+.1f}%)** "
            f"a lo largo de los últimos 7 días.\n\n"
            f"2. **Efectividad del Bloqueo Semanal:** Se han identificado **{len(incompatibles)} parejas símbolo-estrategia** "
            f"que arrojaron pérdidas acumuladas en esta semana. Estas parejas han sido vaciadas e inyectadas de forma real "
            f"en la tabla `symbolNotStrategia` en MySQL.\n\n"
            f"3. **Protección Activa:** El bot en producción evitará operar en estos escenarios de expectativa matemática negativa intradiaria temporal, "
            f"concentrando la liquidez del portafolio en lógicas de alta probabilidad de la semana (ej. Sniper y lógicas SMC)."
        )
        pdf.multi_cell(0, 5, comparativaText.encode('latin-1', 'replace').decode('latin-1'), 0, 'L')
        pdf.ln(5)

        # Banner de arranque semanal
        ...
        pdf.set_font('Helvetica', 'B', 9)
        pdf.set_text_color(197, 160, 89)
        pdf.cell(0, 5, f"   [+] SISTEMA BLINDADO SEMANAL | Balance Compuesto: ${balanceFinal:.2f} USD", 0, 1, 'L')
        pdf.set_font('Helvetica', 'I', 8)
        pdf.set_text_color(180, 220, 200)
        pdf.cell(0, 5, "   Exclusiones semanales recalibradas en la base de datos de MySQL e informe de auditoría completo.", 0, 1, 'L')
