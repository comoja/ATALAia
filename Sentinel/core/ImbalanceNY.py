...
class ImbalanceNYBot:
    ...
    async def _getSignals(self, datos5min: pd.DataFrame, symbolInfo: Dict) -> list:
        ...
        if symbol.upper() not in self.SUPPORTED_SYMBOLS:
            logger.debug(f"[{symbol}] IMBALANCE only para XAU/USD")
            return []
        """
        ...
        logger.info(f"[ImbalanceNY] Precio actual: {precioActual}, Max: {precioMaximo}, Min: {precioMinimo}")
        
        if self.signalGenerada:
            logger.info(f"[ImbalanceNY] Señales ya generadas anteriormente")
            return []
        
        if self.velaCorte is None:
            velaCorte = self.findVelaCorte(datos5min, precioMaximo, precioMinimo)
            if velaCorte is None:
                logger.info(f"[ImbalanceNY] No hay vela de corte todavía")
                return []
            
            self.velaCorte = velaCorte
            logger.info(f"[ImbalanceNY] Vela de corte detectada: {velaCorte['type']} en idx {velaCorte['idx']}")
        ...
        if not fvgs:
            logger.info(f"[ImbalanceNY] No se encontraron FVGs después de vela de corte")
            return []
        
        logger.info(f"[ImbalanceNY] FVGs encontrados: {len(fvgs)} (dentro: {len(fvgs_dentro)}, fuera: {len(fvgs_fuera)})")
        ...
        for idx, fvg in enumerate(fvgs):
            logger.info(f"[ImbalanceNY] FVG {idx+1}: {fvg['type']}, idx: {fvg['idx']}")
    ...
    async def _executeTrades(self, signal: Dict, symbolInfo):
        ...
        if not self.accounts:
            ...
            if not self.accounts:
                logger.warning("[ImbalanceNY] No hay cuentas disponibles")
                return
        ...
        for account in self.accounts:
            ...
            if account['idCuenta'] != 1:
                ...
                logger.info(f"✅ Alerta ImbalanceNY enviada para {symbolInfo['symbol']} a la cuenta {account['idCuenta']}")
        ...
        if fvg_num == 1:
            ...
            logger.info(f"[ImbalanceNY] Marcando señal 1 como enviada a las {ahora}")
        elif fvg_num == 2:
            self.signal2_enviada = True
            self.timestamp_signal2 = ahora
            logger.info(f"[ImbalanceNY] Marcando señal 2 como enviada a las {ahora}")

    async def runAnalysisCycleForSymbol(self, symbolInfo: Dict, preloadedData: Dict = None, apiKey: str = None):
        logger.info(f"[ImbalanceNY] ===== INICIANDO CICLO IMBALANCE =====")
        
        symbol = symbolInfo['symbol']
        
        datos5min = preloadedData.get(symbol) if preloadedData else None
        if datos5min is None or len(datos5min) < 1:
            logger.warning(f"[ImbalanceNY] Datos ({len(datos5min)} velas)insuficientes para {symbol}")
            return
        
        logger.info(f"[ImbalanceNY] Velas recibidas: {len(datos5min)}, desde: {datos5min.index[0]} hasta: {datos5min.index[-1]}")
        ...
        if signals:
            ...
            logger.info(f"[{symbol}] Sin señales ImbalanceNY en este ciclo")
