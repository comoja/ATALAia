    ...
    if isMeanCross and active_trades:
        ...
        for t in active_trades:
            ...
            cycle_trades.append({
                ...
                "multA": t["multA"], "multB": t["multB"],
                "unitsA": int(t["unitsA"]), "unitsB": int(t["unitsB"]),
                "pipsA": round(float(pipsA), 1), "pipsB": round(float(pipsB), 1),
                "pnlA": round(float(pnlA), 2), "pnlB": round(float(pnlB), 2),
                "entryPriceA": round(float(t["entryPxA"]), 5), "exitPriceA": round(float(pxA), 5),
                "entryPriceB": round(float(t["entryPxB"]), 5), "exitPriceB": round(float(pxB), 5),
        ...
        finished_trades.append({
            ...
            "multA": None, "multB": None,
            "unitsA": int(cycle_units_a), "unitsB": int(cycle_units_b),
            ...
            "entryPriceA": None, "exitPriceA": None,
            "entryPriceB": None, "exitPriceB": None,
...
if active_trades:
    ...
    for t in active_trades:
        ...
        open_cycle_trades.append({
            ...
            "pipsA": round(float(pipsA), 1), "pipsB": round(float(pipsB), 1),
            "pnlA": round(float(pnlA), 2), "pnlB": round(float(pnlB), 2),
            "entryPriceA": round(float(t["entryPxA"]), 5), "exitPriceA": round(float(lastPxA), 5),
            "entryPriceB": round(float(t["entryPxB"]), 5), "exitPriceB": round(float(lastPxB), 5),
    ...
    finished_trades.append({
        ...
        "direction": f"● En Curso ({len(open_cycle_trades)} ops)",
        "entryDate": f"Entradas: {openEntrySpan}",
        "exitDate": "EN CURSO",
        "allocatedCapital": round(float(open_margin), 2),
        "multA": None, "multB": None,
        "unitsA": int(open_units_a), "unitsB": int(open_units_b),
        ...
        "returnPct": round((open_pnl / open_margin) * 100.0, 2) if open_margin > 0 else 0.0,
        "pnl": round(float(open_pnl), 2),
        "exitReason": "FLOTANTE_ACTUAL",
        "isWin": bool(open_pnl > 0)
    })

print(f"=== POSICIONES ABIERTAS (EN CURSO) AÑADIDAS A LA BITÁCORA ===")
for t in finished_trades[-4:]:
    print(t)
