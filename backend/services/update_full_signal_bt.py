...
new_signal_engine = '''    # =========================================================================
    # 7. BACKTESTING DE TODAS LAS SEÑALES GRÁFICAS (TRIÁNGULOS, CUADROS Y CÍRCULOS)
    # =========================================================================
    def runSignalBacktest(
        ...
        """
        Ejecuta el backtest capturando TODOS los movimientos y señales visuales:
        - Triángulos (Coincidentes) y Cuadros (Independientes con regla de equidad).
        - Rastrea cada señal individualmente hasta su salida en el siguiente Círculo de Media.
        """
        ...
        for i in range(1, n):
            ...
            if active_trades:
                ...
                for t in active_trades:
                    ...
                    unrealizedTotalPnl += (equity * weight) * tradeNetRet
            ...
            # 2. EVALUAR NUEVA SEÑAL DE ENTRADA
...
import re
code = re.sub(
    r'    # =========================================================================\n    # 7\. BACKTESTING DE SEÑALES GRÁFICAS.*?(?=quantEngine = QuantPairEngine\(\))',
    new_signal_engine + '\n\n',
    code,
    flags=re.DOTALL
)
...
print("Updated quant_pair_engine.py with full visual signal tracking!")
