...
new_engine_20pct = '''    # =========================================================================
    # 7. BACKTESTING CON ASIGNACIÓN DEL 20% BASADO EN symbols.min_lots DE BD
    # =========================================================================
    def runSignalBacktest(
        ...
        """
        Ejecuta el backtest asignando un 20% del capital de la cuenta en cada entrada
        (10% en Par A y 10% en Par B), ajustando los volúmenes a los lotes mínimos (min_lots)
        registrados en la base de datos de MySQL para cada símbolo.
        """
        ...
        for i in range(1, n):
            ...
            # 1. SI HAY CRUCE CON LA MEDIA (CÍRCULO), CERRAMOS TODOS LOS TRADES ACTIVOS
            if isMeanCross and active_trades:
                for t in active_trades:
                    ...
                    finished_trades.append({
                        "tradeNum": len(finished_trades) + 1,
            ...
            if sigType and direction:
                # 20% de capital distribuido en 10% Par A y 10% Par B
                capA = (equity * allocationRate) / 2.0
                capB = (equity * allocationRate) / 2.0
                
                # Ajuste por min_lots de base de datos
                # Si min_lots representa unidades base (ej. 1000 para microlotes), validamos que no sea inferior
...
import re
code = re.sub(
    r'    # =========================================================================\n    # 7\. BACKTESTING DE TODAS LAS SEÑALES GRÁFICAS.*?(?=quantEngine = QuantPairEngine\(\))',
    new_engine_20pct + '\n\n',
    code,
    flags=re.DOTALL
)
...
print("Updated quant_pair_engine.py with 20% allocation and min_lots integration!")
