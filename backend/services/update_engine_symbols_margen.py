...
new_engine_with_symbols_margen = '''    # =========================================================================
    # 7. BACKTESTING BASADO EN symbols.margen, symbols.min_lots Y cuenta.Capital
    # =========================================================================
    def runSignalBacktest(
        ...
        allocationPct: float = 20.0, # 20% de margen de cuenta por entrada (10% Par A + 10% Par B)
        minLotsA: float = 1000.0,    # symbols.min_lots de BD para Par A
        minLotsB: float = 1000.0,    # symbols.min_lots de BD para Par B
        margenPctA: float = 0.25,    # symbols.margen (%) de BD para Par A (ej. 0.25% -> 400:1)
        margenPctB: float = 1.0,     # symbols.margen (%) de BD para Par B (ej. 1.00% -> 100:1)
        commissionBps: float = 2.0,
        slippageBps: float = 1.0
    ) -> Dict[str, Any]:
        """
        Ejecuta el backtest calculando el apalancamiento y margen de garantía
        estrictamente a partir de symbols.margen y symbols.min_lots de MySQL.
        """
        ...
        raw_trades_count = 0
        cycle_counter = 0
        equity = float(initialCapital) if initialCapital > 0 else 10000.0
        peakEquity = equity
        maxDrawdown = 0.0
        equityCurve = []
        ...
        for i in range(1, n):
            ...
            # Calcular PnL no realizado de todos los trades activos con apalancamiento symbols.margen
            ...
            if isMeanCross and active_trades:
                ...
                for t in active_trades:
                    ...
                    cycle_trades.append({
                        ...
                        "exitDate": d,
                        "allocatedCapital": round(float(t["margin"]), 2), # Margen invertido según symbols.margen
                ...
                finished_trades.append({
                    ...
                    "exitDate": d,
                    "allocatedCapital": round(float(cycle_margin), 2), # Margen Total del Cierre
            ...
            # 2. EVALUAR ENTRADA CON symbols.margen Y symbols.min_lots
            ...
            if sigType and direction:
                ...
                # Margen requerido por 1 lote mínimo usando symbols.margen (%)
                margen1LotA = (minLotsA * pxA) * margenRateA
                margen1LotB = (minLotsB * 1.0) * margenRateB

                multA = max(1, int(round(targetMargenA / margen1LotA))) if margen1LotA > 0 else 1
                multB = max(1, int(round(targetMargenB / margen1LotB))) if margen1LotB > 0 else 1
...
import re
code = re.sub(
    r'    # =========================================================================\n    # 7\. BACKTESTING CON APALANCAMIENTO.*?(?=quantEngine = QuantPairEngine\(\))',
    new_engine_with_symbols_margen + '\n\n',
    code,
    flags=re.DOTALL
)
...
print("Updated quant_pair_engine.py with symbols.margen percentage integration!")
